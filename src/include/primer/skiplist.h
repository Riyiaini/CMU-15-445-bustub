...
class SkipList {
 ...
 public:
  ...
  explicit SkipList(const Compare &compare = Compare{}) : compare_(compare), height_(1), size_(0) {
...
SKIPLIST_TEMPLATE_ARGUMENTS struct SkipList<K, Compare, MaxHeight, Seed>::SkipNode {
   ...
   */
  explicit SkipNode(size_t height, K key = K{}) : key_(std::move(key)) { 
    links_.resize(height, nullptr);
  }
